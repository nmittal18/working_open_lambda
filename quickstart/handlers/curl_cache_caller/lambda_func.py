...
def handler(conn, event):
	...
	fd = open("curl_latency_cache_run_" + str(l) + ".txt", 'wb')
	f = open("curl_cache_output_" + str(l) + ".txt", 'wb')
	...
	for _ in range(i):
		start = time.time()
		call(["curl", "http://172.17.0.1:8080/runLambda/hello", "-d", payload ], stdout=f, stderr=f)
		end = time.time()
		elapsed = end - start
